...
Page({
    ...
    openGoodsModal(e) {
        var storeIndex = e.currentTarget.dataset.store
        var goodsList = this.data.order.storeShippingCartList[storeIndex].cartList
        this.setData({selectGoodsList: goodsList})
        this.setData({ isGoodShow: true})
    },

    closeGoodsModal() {
        this.setData({selectGoodsList: []})
        this.setData({ isGoodShow: false})
    },

    openInvoiceModal() {
        this.setData({ isInvoiceShow: true})
    },

    closeInvoiceModal() {
        this.setData({ isInvoiceShow: false})
    },

    onLoad: function (options) {
        if (wx.getStorageSync('cart2order')) {
            var data = wx.getStorageSync('cart2order');
            this.setData({ goods: JSON.parse(data) });
            wx.setStorageSync('cart2order', '')
        }
        this.requestCart2(0);
    },
    ...
    // 选择开发票 不开 开 
    choseInvoice: function (e) {
        this.setData({ 'invoice.is_use': e.target.dataset.isneed == "1" });
    },

    // 选择发票抬头 个人 单位
    chosePerson: function (e) {
        this.setData({ 'invoice.is_person': e.target.dataset.isper == "1" });
    },
    ...
    calculatePrice: function (formData, submitOrder) {
        ...
        if (this.data.goods.action) {
            postData = {
                ...
                pay_points: formData.pay_points || 0,
                user_money: formData.user_money || 0,
                pwd: pwd,
                pay_pwd: pwd,
        ...
        postData = Object.assign(postData, that.getInvoiceData(formData), {
            share_id: that.data.goods.share_id
        });
        request.post('/api/cart/cart3', {
            ...
            success: function (res) {
                ...
                // wx.setStorageSync(`orderId:${res.data.result}`, postData);
                common.jumpToCart4({
