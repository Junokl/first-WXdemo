...
Page({
            ...
            that.uploadPhotos(function () {
                request.post('/api/order/return_goods', {
                    data: {
                        ...
                        is_receive: (that.data.typeBtnIndex == 1 || that.data.typeBtnIndex == 2 || that.data.typeBtnIndex == 3) ? 1: Number(that.data.isReceive),
    ...
    checkSubmit: function(call) {
        if (this.data.checkCauseIndex < 0) {
            return app.showTextWarining("请先选择提交原因");
        }
        if (this.data.description.length == 0) {
            return app.showTextWarining("请先填写问题描述");
        }
        if (this.data.filePaths.length == 0) {
            return wx.showModal({
                title: '确定不上传图片吗？',
