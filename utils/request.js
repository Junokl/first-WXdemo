...
module.exports = {
    ...
    requestUser: function (method, url, options) {
        var that = this;
        //设置默认头部
        var header = options.header ? options.header : { "content-type": "application/x-www-form-urlencoded" };
        //设置请求方式并做相应处理
        ...
        var data = (method != 'GET' && options.data) ? util.json2Form(options.data) : options.data;
        //处理请求url
        url = this.modifyUserUrl(url, options);
        //是否显示加载图标
        options.isShowLoading = typeof options.isShowLoading == 'undefined' ? true : options.isShowLoading;
        options.isShowLoading && this.showLoading();
        console.log('app.request', url, options);
    ...
    /** get请求,说明同request() */
    getUser: function (url, options) {
        this.requestUser('GET', url, options);
    },
    
    /** post请求,说明同request() */
    postUser: function (url, options) {
        this.requestUser('POST', url, options);
    },
    ...
    /** 加基地址,附加鉴权参数 */
    modifyUserUrl: function (url, options) {
        typeof options == 'undefined' && (options = {});
        if (url.indexOf('http') != 0) {
            if (typeof options.baseUrl == 'string') {
                url = options.baseUrl + url;
                ...
                var app = getApp();
                url = app.globalData.setting.userUrl + url;
            }
        }
        if (typeof options.notAuthParam == 'boolean' && options.notAuthParam == true) {
            return url;
